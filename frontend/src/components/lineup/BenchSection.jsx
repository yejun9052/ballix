...
export function BenchSection({ events, lineup, loading, match, onSelectPlayer }) {
  if (loading) {
  ...
  return (
    <div className="bench-grid">
      <BenchList team={match.homeTeam} players={sortBench(homeBench)} events={events} onSelect={onSelectPlayer} />
      <BenchList team={match.awayTeam} players={sortBench(awayBench)} events={events} onSelect={onSelectPlayer} />
    </div>
  );
}

export function BenchList({ events, players, team, onSelect }) {
  return (
    <div className="bench-list">
      ...
      {players.length === 0 ? (
        ...
        players.map((player) => {
          ...
          return (
            <span
              className={`bench-row ${subIn !== null ? "came-in" : ""} ${clickable ? "is-clickable" : ""}`}
              key={player.id}
