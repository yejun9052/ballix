...
import { matchAdminApi } from "../../services/api.js";

export function ReplayPanel({ match, isAdmin }) {
  const [replayId, setReplayId] = useState(match.raw?.replayYoutubeId || "");
  ...
  async function handleSave() {
    ...
    setError("");
    try {
      const updated = await matchAdminApi.setReplay(match.id, input.trim());
  ...
  async function handleClear() {
    ...
    setError("");
    try {
      await matchAdminApi.clearReplay(match.id);
      setReplayId("");
    } catch (e) {
