...
// FotMob positionId 십의 자리 = 라인(포메이션과 무관하게 고정): 1x=GK, 3x=수비, 6x/7x=미드, 8x=공격형 미드, 10x+=공격.
// 좌표 깊이(posX)는 라인 수에 따라 간격이 달라져(예: GK는 항상 posX=0.1) 깊이 임계값으로는 GK가 CB로, 미드가 CAM/ST로 오분류된다.
// 그래서 라인은 positionId로 정한다(좌우 lateral 규약은 getDetailedLabel과 동일). positionId 없으면 null.
export function labelFromPositionId(positionId, lateral) {
  if (!Number.isFinite(positionId)) return null;
  if (positionId < 30) return "GK";
  const isR = lateral < 0.28;
  const isL = lateral > 0.72;
  if (positionId < 50) return isR ? "RB" : isL ? "LB" : "CB";    // 수비
  if (positionId < 80) return isR ? "RM" : isL ? "LM" : "CM";    // 중앙/수비형 미드
  if (positionId < 100) return isR ? "RW" : isL ? "LW" : "CAM";  // 공격형 미드·윙
  return isR ? "RW" : isL ? "LW" : "ST";                         // 최전방
}

// posX(깊이) + posY(좌우: 0=오른쪽, 1=왼쪽) → 상세 포지션 라벨 (positionId가 없을 때의 폴백)

export function getDetailedLabel(depth, lateral) {
  const isR = lateral < 0.28;
  const isL = lateral > 0.72;

  if (depth <= 0.12)  return "GK";
...
export function getPlayerLayout(player) {
  ...
  if (Number.isFinite(player.posX) && Number.isFinite(player.posY)) {
    ...
    return { depth, lateral, label: resolveLabel(player, lateral, depth) };
  }
...
// 포지션 라벨 우선순위: positionId(라인 고정, 가장 정확) → 백엔드 저장값(player.position) → 좌표 깊이 폴백.
function resolveLabel(player, lateral, depth) {
  return (
    labelFromPositionId(player.positionId, lateral) ||
    player.position ||
    getDetailedLabel(depth, lateral)
  );
}
